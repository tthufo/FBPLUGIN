...
//    }
    ...
    if(_isNavigation && !isRemove)
    {
        ...
        UIImageView * prev = [[UIImageView alloc] initWithFrame:CGRectMake(2, 7, [self.tabHeight floatValue] - 14, [self.tabHeight floatValue] - 14)];
        prev.image = [UIImage imageNamed:@"ic_previous_photo"];
        prev.tag = 96;
...
- (void)setActiveTabIndex:(NSUInteger)activeTabIndex {
    ...
    if (isMultiColor)
    {
//        for(UIView * v in self.tabsView.subviews)
//        {
//            if([v isEqual:tabView])
//            {
//                ((UILabel*)[((TabView*)v).subviews firstObject]).textColor = [UIColor greenColor];
//            }
//            else
//            {
//                ((UILabel*)[((TabView*)v).subviews firstObject]).textColor = [UIColor whiteColor];
//            }
//        }
        for(UIView * v in self.tabsView.subviews)
        {
            if([self.tabsView.subviews indexOfObject:v] == [self.indexSelected intValue])
            {
                ((UILabel*)[((TabView*)v).subviews firstObject]).textColor = [UIColor greenColor];
            }
            else
...
- (void)setNeedsReloadOptions {
    ...
    for (NSUInteger i = 0; i < self.tabCount; i++) {
        ...
        frame.size.width = self.arr.count == 0 ? [self.tabWidth floatValue] : [self.arr[i] floatValue];
        tabView.frame = frame;
...
- (void)reloadTabView
{
    ...
    for (NSUInteger i = 0; i < self.tabCount; i++)
    {
        ...
        CGRect frame = tabView.frame;
        frame.origin.x = contentSizeWidth;
        frame.size.width = self.arr.count == 0 ? [self.tabWidth floatValue] : [self.arr[i] floatValue] + 15;
        tabView.frame = frame;
        
        if(_isSeparator && i != 0)
        {
            UIView * line = [[UIView alloc] initWithFrame:CGRectMake(0, 8, 1, [self.tabHeight floatValue] - 16)];
    ...
    [self setActiveTabIndex:[indexSelected intValue]];
}
...
//    self.tabsView.frame = frame;
    ...
    // Select starting tab
    NSUInteger index = [self.startFromSecondTab boolValue] ? [self.startFromSecondTab intValue] : 0;
    
    [self selectTabAtIndex:indexSelected ? [indexSelected intValue] : index];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    if ([self.delegate respondsToSelector:@selector(willScroll)])
    {
        [self.delegate  willScroll];
    }
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    {
        ((UIImageView*)[self withView:self.view tag:96]).alpha = 0;
        ((UIImageView*)[self withView:self.view tag:69]).alpha = 1;
    }
    else if (self.activeTabIndex == self.tabCount - 1)
    {
        ((UIImageView*)[self withView:self.view tag:96]).alpha = 1;
        ((UIImageView*)[self withView:self.view tag:69]).alpha = 0;
    }
    else
    {
        ((UIImageView*)[self withView:self.view tag:96]).alpha = 1;
